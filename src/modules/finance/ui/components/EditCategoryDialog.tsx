...
import { UpdateCategoryInput } from "@/modules/finance/domain/FinanceRepository";
...
export function EditCategoryDialog({
  ...
  category: Category;
  onClose: () => void;
  onSubmit: (input: UpdateCategoryInput) => Promise<boolean>;
  submitting: boolean;
  error: string | null;
}) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/70 p-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-surface-1 p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-headline-md font-semibold text-text-1">Editar categoría</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block font-label text-label-md text-text-3">Color</label>
            <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="h-10 w-full rounded-lg border border-border bg-surface-1" />
          </div>
